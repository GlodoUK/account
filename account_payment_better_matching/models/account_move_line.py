...
class AccountMoveLine(models.Model):
    ...
    @api.multi
    def _reconcile_lines_partial(self, debit_moves, credit_moves, field):
        ...
        while (debit_moves and credit_moves):
            ...
            override_move = False
            if debit_move.reconcile_override and debit_move.reconcile_override > 0 and debit_move.reconcile_override < amount_reconcile:
                override_move = debit_move
            elif credit_move.reconcile_override and credit_move.reconcile_override > 0 and credit_move.reconcile_override < amount_reconcile:
                override_move = credit_move

            if override_move:
                amount_reconcile = override_move.reconcile_override
                temp_amount_residual = override_move.currency_id._convert(override_move.reconcile_override,override_move.company_id.currency_id,override_move.company_id,override_move.date)
    ...
    @api.multi
    def auto_reconcile_lines_partial(self):
    ...
    @api.multi
    def partial_reconcile(self, writeoff_acc_id=False, writeoff_journal_id=False):
        ...
        remaining_moves = self.auto_reconcile_lines_partial()
